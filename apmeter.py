...
class APMeter(meter.Meter):
    ...
    def value(self):
        ...
        for k in range(self.scores.size(1)):
            ...
            sort_scores, sortind = torch.sort(scores, 0, True)
            truth = targets[sortind]
            truthsum = truth.float().cumsum(0)
            #True positive
            tp = (sort_scores>=0.05)&(truth==1.0)
            fp = (sort_scores>=0.05)&(truth==0.0)
            #print(tp)
            tp_cum = tp.float().cumsum(0)
            fp_cum = fp.float().cumsum(0)
            precision = tp_cum.div(tp_cum+fp_cum)
            tot_tp = max(tp.float().sum(),1)
            #print(k,tp.float().sum(), fp.float().sum())
            ap[k] = precision[tp.float().byte()].sum() / tot_tp
            ...
            ap[k] = precision[truth.float().byte()].sum() / max(truth.float().sum(), 1)
            '''
        return ap
