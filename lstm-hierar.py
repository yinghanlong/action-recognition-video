...
class LSTM_model(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, num_layer,num_classes,batch=1,enhance=0):
        ...
        self.outmap = nn.Linear(25, num_classes,bias=False).cuda()
    ...
    def forward(self, video):#
        ...
        output_ori = torch.zeros(len(video),self.num_classes).cuda()
        ...
        for i in range(len(video)):
            ...
            if i>=5:#self.enhance and enhance_cnt>=enhance_capa:
                ...
                avg_out = torch.cat((tag_space[i-5,:].view(1,-1),tag_space[i-4,:].view(1,-1),tag_space[i-3,:].view(1,-1),tag_space[i-2,:].view(1,-1),tag_space[i-1,:].view(1,-1)),0)
                #print(avg_out.size())
                #avg_out= torch.mean(tag_space[i-5:i+1,:],dim=0,keepdim=True).cuda()
                general_out = torch.cat((avg_out[:,23:26],avg_out[:,43].view(-1,1),avg_out[:,62].view(-1,1)),1)#torch.zeros((6)).cuda()
                #24 stand, 25 run, 26 jump, 36 throw, 44 squat, 63 talktocamera
        ...
        output_ori = output_ori.t()
        output_ori = output_ori.view(1,self.num_classes,-1)
        
        return output_ori, tag_space #(batch_size, num_classes,seq_length)
...
train_data,train_labels,validation_data,validation_labels,resume,batch=1,dataset='ucf'):
        ...
        if dataset=='ucf':
            ...
            lr_patience = 10
            self.scheduler = MultiStepLR(self.optimizer, [35])
        ...
        for self.epoch in range(self.nb_epochs):
            ...
            if is_best:
            	...
            	if self.enhance:
                    ...
                    'best_prec1': self.best_prec1,
                    'optimizer' : self.optimizer.state_dict()
                    },is_best,'record/checkpoint.pth.tar','record/'+self.dataset+'_hierar.pth.tar')
    ...
    def train_1epoch(self):
        ...
        # Forward propagation for all frames in the video
           ...
           # Step 3. Run our forward pass.
            output_lstm, tag_scores = self.model(cnn_out)
    ...
    def validate_1epoch(self):
        ...
        with torch.no_grad():
          ...
          for name in self.validation_data.keys(): 
            ...
            output_lstm, output = self.model(data_var)
            ...
            preds2 = output_lstm.data.cpu().numpy()
            preds2 = preds2.reshape((self.num_classes,-1))
            preds2 = np.transpose(preds2)
            results2=torch.from_numpy(preds2).float() 
            ...
            if self.dataset=='multithumos':
                ...
                results2 = torch.sigmoid(results2)
                #Average precision meter
                apm2.add(results2, labels)
            else:
        ...
        if self.dataset =='ucf':
            ...
            info = {'Epoch':[self.epoch],
                ...
                'Loss':[round(losses.avg,5)],
                'Prec@1':[round(apm.value().mean(),4)],
                'Prec@5':[round(apm2.value().mean(),4)]
                }
            ...
            print 'mAP=',apm.value().mean(),apm2.value().mean()
            print 'AP=',apm.value(),apm2.value()
            print 'loss=',round(losses.avg,5)#loss.data.cpu()
