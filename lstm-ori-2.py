...
class LSTM_model(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, num_layer,num_classes,batch=1,enhance=0):
        ...
        self.outmap = nn.Linear(6, num_classes).cuda()
    ...
    def forward(self, video):#
        ...
        for i in range(len(video)):
            ...
            if self.enhance and enhance_cnt>=enhance_capa:
                ...
                #zi= torch.mul(g[0],f[0]) + torch.mul(g[1],f[1]) +torch.mul(g[2],f[2]) +torch.mul(g[3],f[3]) +torch.mul(g[4],f[4]) + torch.mul(g[5],f[5]) + torch.mul(g[6],f[6]) +torch.mul(g[7],f[7]) +torch.mul(g[8],f[8]) +torch.mul(g[9],f[9])+ torch.mul(g[10],f[10]) + torch.mul(g[11],f[11]) +torch.mul(g[12],f[12]) +torch.mul(g[13],f[13]) +torch.mul(g[14],f[14]) + torch.mul(g[15],f[15]) + torch.mul(g[16],f[16]) +torch.mul(g[17],f[17]) +torch.mul(g[18],f[18]) +torch.mul(g[19],f[19])
        		#Original input + zi
        ...
        for self.epoch in range(self.nb_epochs):
            ...
            if is_best:
            	...
            	if self.enhance:
                    ...
                    },is_best,'record/checkpoint.pth.tar','record/'+self.dataset+'_enhance_separate.pth.tar')
                else:
                    save_checkpoint({
                    'epoch': self.epoch,
                    'state_dict': self.model.state_dict(),
                    'best_prec1': self.best_prec1,
                    'optimizer' : self.optimizer.state_dict()
                    },is_best,'record/checkpoint.pth.tar','record/'+self.dataset+'_ori-2.pth.tar')
    ...
    if arg.evaluate == False:
      for name in sorted(opf.keys()):   
        ...
        if videoName not in video_preds.keys():
            ...
            cut_num = int(sample_num/MAX_CLIP)               
              
            for i in range(cut_num):
                videoid= videoName + str(i)
                video_preds[videoid] = torch.zeros((MAX_CLIP,BATCH_SIZE, 2*len(r))).cuda()#preds[j,:]
                if arg.dataset=='ucf':
                    video_labels[videoName] = torch.zeros((BATCH_SIZE,MAX_CLIP),dtype=torch.long)#preds[j,:]
                else:
                    video_labels[videoid] = torch.zeros((BATCH_SIZE,num_classes,MAX_CLIP),dtype=torch.float)

            videoid= videoName + str(cut_num)
            video_preds[videoid] = torch.zeros((sample_num-cut_num*MAX_CLIP,BATCH_SIZE, 2*len(r))).cuda()
            video_labels[videoid] = torch.zeros((BATCH_SIZE,num_classes,sample_num-cut_num*MAX_CLIP),dtype=torch.float)

        videoid= videoName + str(int(clip_idx/MAX_CLIP))
        clip_idx -= MAX_CLIP*int(clip_idx/MAX_CLIP)
        #set input to lstm and labels
        video_preds[videoid][clip_idx,0,0:len(r)] = torch.squeeze(torch.from_numpy(r)).cuda()
        video_preds[videoid][clip_idx,0,len(r):2*len(r)] = torch.squeeze(torch.from_numpy(o)).cuda() #[clip_idx,batch,input_dim] 
        if arg.dataset=='ucf':  
            video_labels[videoName][:,clip_idx] = label
        else:
            frame_idx=clip_idx*5
            video_labels[videoid][0,:,clip_idx] = label[frame_idx,:]
    ...
    for name in sorted(opf_v.keys()):   
        ...
        if videoName not in validation_preds.keys():
            ...
            for i in range(cut_num):
                ...
                validation_preds[videoid] = torch.zeros((MAX_CLIP,BATCH_SIZE, 2*len(r))).cuda()#preds[j,:]
                if arg.dataset=='ucf':
                    validation_labels[videoName] = torch.zeros((BATCH_SIZE,sample_num),dtype=torch.long)
                else:
                    validation_labels[videoid] = torch.zeros((BATCH_SIZE,num_classes,MAX_CLIP),dtype=torch.float)
            videoid= videoName + str(cut_num)
            validation_preds[videoid] = torch.zeros((sample_num-cut_num*MAX_CLIP,BATCH_SIZE, 2*len(r))).cuda()
            validation_labels[videoid] = torch.zeros((BATCH_SIZE,num_classes,sample_num-cut_num*MAX_CLIP),dtype=torch.float)

        #set validation set inputs and labels
        videoid= videoName + str(int(clip_idx/MAX_CLIP))
        clip_idx -= MAX_CLIP*int(clip_idx/MAX_CLIP)
        validation_preds[videoid][clip_idx,0,0:len(r)] = torch.squeeze(torch.from_numpy(r)).cuda()
        validation_preds[videoid][clip_idx,0,len(r):2*len(r)] = torch.squeeze(torch.from_numpy(o)).cuda()
        if arg.dataset=='ucf':
            validation_labels[videoName][:,clip_idx] = label
        else:
            frame_idx=clip_idx*5
            validation_labels[videoid][0,:,clip_idx] = label[frame_idx,:]
