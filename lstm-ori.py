...
class LSTM_model(nn.Module):
    ...
    def init_weight(self):
        ...
        print('Initial weights:',self.updatez.weight.data)
...
train_data,train_labels,validation_data,validation_labels,resume,batch=1,dataset='ucf'):
    ...
    def train_1epoch(self):
        ...
        if self.dataset=='ucf':
            ...
            record_info(info, 'record/lstm/opf_train.csv','train')
        else:
            #accuracy
            print 'MAP:', apm.value().mean()
            print 'loss:', round(losses.avg,5)#losses.data.cpu()
    ...
    def validate_1epoch(self):
        ...
        with torch.no_grad():
          ...
          for name in self.validation_data.keys(): 
            ...
            results=torch.from_numpy(preds).float()
            if self.dataset=='multithumos':
        ...
        if self.dataset =='ucf':
            ...
            record_info(info, 'record/lstm/opf_test.csv','test')
...
#validation
    ...
    if arg.top5enhance:
        INPUT_DIM = 2*len(r)#+2*num_classes # dimension of input from CNN
    else:
        INPUT_DIM = 2*len(r)
